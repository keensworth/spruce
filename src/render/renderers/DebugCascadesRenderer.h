...
namespace spr::gfx {
class DebugCascadesRenderer {
public:
    DebugCascadesRenderer(){}
    DebugCascadesRenderer(VulkanResourceManager& rm, VulkanRenderer& renderer, glm::uvec3 dimensions){
        m_rm = &rm;
        m_renderer = &renderer;
        m_dim = dimensions;
    }
        ...
        Handle<Buffer> shadowData)
    {
        ...
        m_shader = m_rm->create<Shader>({
            .vertexShader   = {.path = "../data/shaders/spv/debug_cascades.vert.spv"},
            .fragmentShader = {.path = "../data/shaders/spv/debug_cascades.frag.spv"},
            .descriptorSets = {
                { globalDescSetLayout },
                { frameDescSetLayout },
    ...
    void destroy(){
        ...
        SprLog::info({{"[DebugCascadesRenderer] ", color::GRADIENT18}, {"[destroy] destroyed..."}});
    }
