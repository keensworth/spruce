...
public:
    ...
    void init(
        ...
        Handle<DescriptorSet> frameDescriptorSets[MAX_FRAME_COUNT],
        Handle<DescriptorSetLayout> frameDescriptorSetLayout)
    {
        ...
        for (uint32 i = 0; i < MAX_FRAME_COUNT; i++)
            m_descriptorSets[i] = frameDescriptorSets[i];
        m_descriptorSetLayout = frameDescriptorSetLayout;

        // color attachment
        m_attachment = m_rm ->create<TextureAttachment>(TextureAttachmentDesc{
            .textureLayout = {
                .dimensions = m_dim,
...
private: // owning
    ...
    Handle<DescriptorSet> m_descriptorSets[MAX_FRAME_COUNT];
