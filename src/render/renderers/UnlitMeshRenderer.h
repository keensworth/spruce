...
namespace spr::gfx {
class UnlitMeshRenderer {
public:
    UnlitMeshRenderer(){}
    UnlitMeshRenderer(VulkanResourceManager& rm, VulkanRenderer& renderer, glm::uvec3 dimensions){
        m_rm = &rm;
        m_renderer = &renderer;
        m_dim = dimensions;
    }
        ...
        Handle<TextureAttachment> depthAttachment)
    {
        ...
        m_shader = m_rm->create<Shader>({
            .vertexShader   = {.path = "../data/shaders/spv/unlit_mesh.vert.spv"},
            .fragmentShader = {.path = "../data/shaders/spv/unlit_mesh.frag.spv"},
            .descriptorSets = {
                { globalDescSetLayout },
                { frameDescSetLayout},
                { }, // unused
    ...
    void destroy(){
        ...
        SprLog::info({{"[UnlitMeshRenderer] ", color::GRADIENT18}, {"[destroy] destroyed..."}});
    }
